...
public class DemoApplication {
    static void connect(String username, String password) {
        ...
        if (true /* 강제로 예외를 발생시키기 위해 if (true) 추가 - 그렇지 않으면 '아래부터 실행되지 않는데 코드가 있다'는 빌드 오류 */) {
    ...
    public static void main(String args[]) {
        try {
            ...
            log.warn("잘못된 유저 정보로만 데이터베이스 접속 실패, e");
        } catch (Exception e) {
            log.warn("데이터베이스 내 알 수 없는 오류 발생 - 원인 분석 및 대응 필요, e");
        }
    }
}